...
//   static String APIURL = "http://13.126.28.220";
...
class CommonFunctions {
  static String APIURL = "http://13.126.28.220"; // your base URL

  static Map<String, String> getHeaders({bool auth = false}) {
    final headers = {'Content-Type': 'application/json'};
    if (auth) headers['token'] = Storage.getJwtToken();
    return headers;
  }

  Future<ReturnObj> getDispatchList() async {
    try {
      final url = Uri.parse('$APIURL/api/order/get-dispatch-list');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        DispatchListResponse dispatchList =
            DispatchListResponse.fromJson(json.decode(response.body));
        return ReturnObj<List<DispatchList>>(
            message: "Fetched Successfully",
            status: true,
            data: dispatchList.dispatchList);
      }
      return ReturnObj(message: "Failed to fetch Dispatch List", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> activateUser(
      {String? authRequestId, bool? isApproved}) async {
    try {
      final url = Uri.parse('$APIURL/api/auth/user-activation');
      ...
      if (response.statusCode == 200) {
        return ReturnObj(
            message: json.decode(response.body)["message"], status: true);
      }
      return ReturnObj(message: "Failed to activate user", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> getUserActivationRequests() async {
    try {
      final url = Uri.parse('$APIURL/api/auth/user-activation-requests');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        UserActivationResponse userActivationResponse =
            UserActivationResponse.fromJson(json.decode(response.body));
        return ReturnObj<List<Request>>(
            message: "Fetched Successfully",
            status: true,
            data: userActivationResponse.requests);
      ...
      return ReturnObj(status: false, message: "Error: $error", data: []);
    }
  }

  Future<ReturnObj> assignSubadmin(String userId, String subadminId) async {
    try {
      final url = Uri.parse('$APIURL/api/auth/add-subadmin-to-user');
      final response = await http.post(url,
          headers: getHeaders(auth: true),
          body: json.encode({
            "userId": userId,
      ...
      return ReturnObj(message: "Failed to assign Subadmin", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> getSubadminList() async {
    try {
      final url = Uri.parse('$APIURL/api/auth/subadmin-list');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        SubadminListResponse subadminListResponse =
            SubadminListResponse.fromJson(json.decode(response.body));
        return ReturnObj<List<Subadmins>>(
            message: "Fetched Successfully",
            status: true,
            data: subadminListResponse.subadmins);
      ...
      return ReturnObj(
          message: "Failed to fetch Subadmins List", status: false, data: []);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error", data: []);
    }
  }
  ...
  Future<ReturnObj> getNotifications() async {
    try {
      final url = Uri.parse('$APIURL/api/notification/all-notifications');
      final response = await http.get(url, headers: getHeaders(auth: true));
      ...
      return ReturnObj(message: "Failed to fetch notifications", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> ResetPassword(
      String password, String confirmPassword) async {
    try {
      final url = Uri.parse('$APIURL/api/auth/reset-password');
      final response = await http.post(url,
          headers: getHeaders(auth: true),
          body: json.encode({
            "password": password,
      ...
      return ReturnObj(message: "Failed to reset password", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> ForgotPassword(String email) async {
    try {
      if (email.isEmpty) {
        return ReturnObj(message: "Please enter your Email", status: false);
      }

      final url = Uri.parse('$APIURL/api/auth/forgot-password');
      final response = await http.post(url,
          headers: getHeaders(),
          body: json.encode({
            "accountNumberOrEmail": email.toLowerCase(),
          }));

      if (response.statusCode == 200) {
        ForgotPasswordResponse forgotPasswordResponse =
            ForgotPasswordResponse.fromJson(json.decode(response.body));
        return ReturnObj(message: forgotPasswordResponse.message, status: true);
      }
      return ReturnObj(
          message: "Failed to send forgot password email", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> AddUsers(List<int> fileBytes) async {
    try {
      final url = Uri.parse('$APIURL/api/auth/add-user');
      ...
      if (response.statusCode == 200) {
        ...
        return ReturnObj(
            status: true,
            message:
                "${data["message"]} Customer List will reflect soon after processing");
      }
      return ReturnObj(message: TextConstants().SERVER_BUSY, status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
  ...
  Future<ReturnObj> Login(String email, String password) async {
    try {
      ...
      final response = await http.post(url,
          headers: getHeaders(),
          body: json
              .encode({"accountNumberOrEmail": email, "password": password}));
      ...
      return ReturnObj(
          message: json.decode(response.body)["message"], status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Login Unsuccessful: $error");
    }
  }
  ...
  Future<ReturnObj> AddSubAdmin(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        return ReturnObj(
            status: true, message: json.decode(response.body)["message"]);
      }
      return ReturnObj(message: TextConstants().SERVER_BUSY, status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
  ...
  Future<ReturnObj> CheckUser(String accountNumberOrEmail) async {
    try {
      ...
      final url = Uri.parse('$APIURL/api/auth/check-user');
      final response = await http.post(url,
          headers: getHeaders(),
          body: json.encode({"accountNumberOrEmail": accountNumberOrEmail}));

      if (response.statusCode == 200) {
        CheckUserResponse checkUserResponse =
            CheckUserResponse.fromJson(json.decode(response.body));
        return ReturnObj(status: true, message: checkUserResponse.message);
      }
      return ReturnObj(message: "Server is busy", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
  ...
  Future<ReturnObj> getOrderDetails(String orderId) async {
    try {
      if (orderId.isEmpty) {
        return ReturnObj(message: "Invalid Order ID", status: false);
      }
      final url =
          Uri.parse('$APIURL/api/order/get-order-details?soNumber=$orderId');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        SingleOrderDetailsResponse details =
            SingleOrderDetailsResponse.fromJson(json.decode(response.body));
        return ReturnObj<List<SingleOrderDetail>>(
            message: "Success", status: true, data: details.singleOrderDetails);
      }
      return ReturnObj(message: "Unable to get Order Details", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }

  Future<ReturnObj<List<OrderPastComment>>> getPastComments(
      String orderId) async {
    try {
      if (orderId.isEmpty) {
        return ReturnObj(message: "Invalid Order ID", status: false);
      ...
      if (response.statusCode == 200) {
        ...
        GetPastComments getPastComments = GetPastComments.fromJson(jsonData);
        ...
        return ReturnObj(
          message: "Failed to fetch past comments",
          status: false,
        );
      }
    } catch (error) {
      return ReturnObj(
        status: false,
        message: "Error fetching past comments: $error",
      );
    }
  }
  ...
  Future<ReturnObj> getDispatchDetails(String orderId) async {
    try {
      ...
      final url =
          Uri.parse('$APIURL/api/order/get-dispatch-details?orderId=$orderId');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        SingleDispatchDetails dispatchDetails =
            SingleDispatchDetails.fromJson(json.decode(response.body));
        return ReturnObj<List<DispatchDetails>>(
            message: "Fetched",
            status: true,
            data: dispatchDetails.dispatchDetails);
      }
      return ReturnObj(message: "Failed to fetch", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> getComments() async {
    try {
      final url = Uri.parse('$APIURL/api/order/order-comments');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        GetComments.GetCommentsResponse comments =
            GetComments.GetCommentsResponse.fromJson(
                json.decode(response.body));
        return ReturnObj<List<GetComments.GetOrderCommentsDetail>>(
            message: "Fetched",
            status: true,
            data: comments.getOrderCommentsDetails);
      }
      return ReturnObj(message: "Failed to fetch comments", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> getOrders() async {
    try {
      final url =
          Uri.parse('$APIURL/api/order/get-order-list?limit=123&skip=0');
      final response = await http.get(url, headers: getHeaders(auth: true));

      if (response.statusCode == 200) {
        OrderListResponse orders =
            OrderListResponse.fromJson(json.decode(response.body));
        return ReturnObj<List<Order>>(
            message: "Fetched", status: true, data: orders.orders);
      }
      return ReturnObj(message: "Failed to fetch orders", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }
  ...
  Future<ReturnObj> addComments(String comment, String orderId) async {
    try {
      ...
      if (response.statusCode == 200) {
        return ReturnObj(message: "Comment added", status: true);
      }
      return ReturnObj(message: "Failed to add comment", status: false);
    } catch (error) {
      return ReturnObj(status: false, message: "Error: $error");
    }
  }

  // Similar pattern you can apply for:
  // getDispatchList, activateUser, getNotifications, assignSubadmin, etc.
}
